...
function ReadButtonOnclick(row) 
{
      ...
      var status =  document.getElementById('dsTable').rows[d].cells[3].innerText;         
      
        if(status === 'pending')
        { $.ajax({
                        type: "POST",
                        url: " <?php echo base_url()?>/index.php/RequestController/updateRequest/acknowledged/0",
                        data: {recId:recId},
                        dataType: "json",
                        success: function(response) {
                            window.location.href = "<?php echo base_url("/index.php/RequestController/viewDetailedRequest"); ?>" +"/"+recId;
                        },
                        error: function(statuserr, exception) {
                            var msg = '';
                            if (statuserr.status === 0) {
